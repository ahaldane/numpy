...
Unneeded whitespace in float array printing removed, 0d repr changes
--------------------------------------------------------------------
...
used to display the sign. Also, the printing of 0d arrays have been overhauled,
to print more like other ndarrays or numpy scalars (see improvements).

These new behaviors can be disabled to mostly reproduce numpy 1.13 behavior by
enabling the "legacy" printing mode, by calling ``np.set_printoptions(legacy=True)``.
