...
``np.set_printoptions`` and ``np.array2string`` will use keyword args only
--------------------------------------------------------------------------
The use of positional args to specify options for these functions is
deprecated. They will eventually only accept keyword arguments.
