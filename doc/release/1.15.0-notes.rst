...
--------------------------------------------------
Previously printing float128 values was buggy, since the special double-double
floating-point-format of float128 on these systems was not accounted for.
float128s now print with correct rounding and uniqueness.

Relatedly, ppc users should upgrade their glibc if it is <=2.23. On ppc, these
versions of glibc's malloc often allocates misaligned memory which can crash
numpy when using float128 values.
