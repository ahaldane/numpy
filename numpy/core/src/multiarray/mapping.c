...
_get_field_view(PyArrayObject *arr, PyObject *ind, PyArrayObject **view)
{
    *view = NULL;
    ...
    /* next check for a list of field names */
        ...
        PyArrayObject *viewref;
        PyArray_Descr *copy_dtype;
        PyObject *copy_dict, *formats;
        int flags;
        PyArray_Descr *fieldtype;
        npy_intp offset;
        ...
        *view = (PyArrayObject*)PyArray_NewFromDescr(
        ...
        for (i = 0; i < seqlen; i++) {
            /* skip most safety checks, since we just created these objects */
            name = PySequence_GetItem(names, i);
            tup = PyDict_GetItem(fields, name);
            if (_unpack_field(tup, &fieldtype, &offset) < 0) {
                Py_XDECREF(*view);
                *view = NULL;
                return 0;
            }
            if (PyList_SetItem(formats, i, (PyObject*)fieldtype) < 0) {
                Py_XDECREF(*view);
                *view = NULL;
                return 0;
            }
        ...
        PyDict_SetItem(copy_dict, PyUString_FromString("names"), names);
        PyDict_SetItem(copy_dict, PyUString_FromString("formats"), formats);
        if (!PyArray_DescrConverter(copy_dict, &copy_dtype)) {
            Py_XDECREF(*view);
            *view = NULL;
            Py_DECREF(copy_dict);
            Py_DECREF(formats);
        ...
        viewref = *view;
        *view = (PyArrayObject *)PyArray_CheckFromAny((PyObject*)viewref,
                                                copy_dtype, 0, 0, flags, NULL);
        Py_XDECREF(viewref);
        return 0;
    }
    return -1;
